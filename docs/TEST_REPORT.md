...
This is a friends/private-room architecture, not public matchmaking or complete ranked anti-cheat. A public TLS/WSS production topology and endpoint are configured in source, but no live AWS/DNS deployment has been verified, so phones on the internet cannot use the local proof yet. Room codes carry only about 30 bits and the rate limit is identity/node-local. Protocol 4 rejects client-authored snapshots/results and keeps race authority in Nakama; production abuse controls and physical-device internet evidence remain open.
...
| Current development-host AI/load gates | PASS—combined component evidence | Schema-2 full-coverage AI soak passed at 2,084,371 ms and 1,407.6 authority vehicle-steps/s; protocol-4 cloud authority passed 401 deterministic network assertions, 89 real E2E assertions, and a 1,002-assertion 12-client load. The historical 12-check restore remains valid separately. No new one-shot `--release` PASS is claimed |
