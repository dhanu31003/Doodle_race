...
Normal mode caps the engine at 60 FPS; Battery Saver caps it at 30 FPS. Low Graphics mode increases generated-track sample spacing, halves the 3D viewport resolution, reduces trackside instances and shadow distance, disables fog/antialiasing, and suppresses transient cosmetic effects. Both camera mounts and their FOV are speed-invariant in every accessibility mode, so acceleration, suspension heave, and gear changes cannot pull the horizon down; Reduced Motion still sharpens camera response and suppresses other presentation motion. These settings do not alter authoritative physics, track geometry, checkpoint visibility, collision fairness, or multiplayer compatibility.

The mobile tier now also bounds built-in track generation to at most 460 mesh segments and 5,520 triangles, coarsens trackside barrier/fence cadence, and caps distance-driven detail. Remote Formula cars share a single authored-body `ArrayMesh` draw and render their four wheels through one `MultiMesh` draw; the remote tier is approximately 21 nodes, six meshes, and 8,824 triangles, versus the locally controlled high-detail car's 148 nodes and 124 meshes. Remote shadows are disabled, while the player's visual quality remains intact.
...
Cockpit-only interior microdetails are visibility-switched rather than rebuilt. Chase view hides 90 player-only interior details synchronously, and cockpit view restores them synchronously. The driver's hands and sleeves were raised and constrained above a conservative cockpit-floor guard through full left/right steering lock, eliminating the body intersection shown in the original report.
...
All six built-ins are exactly 2.5 times their previous authority length and 12 authority units wider. Physical lap lengths now range from 1,199.976 m to 1,379.969 m and widths from 13.2 m to 14.4 m. The current catalog benchmark recorded six-track cold construction in 412.991 ms, mesh compilation in 6,168.768 ms, a 1,274.243 ms maximum individual track, and 120 warm lookups in 11.978 ms. Generation occurs before active racing; it is not represented as a race-time frame cost.

Evidence: [`../evidence/logs/full-check-20260724T173534Z.log`](../evidence/logs/full-check-20260724T173534Z.log), SHA-256 `e0b9d42ff06398a85b755b25ff715284bfabb921f01a947bc3ed74c3be369690`.
