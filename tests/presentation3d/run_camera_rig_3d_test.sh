...
TEST_LOG="$(mktemp -t raceglyph-camera-rig-3d.XXXXXX)"
trap 'rm -f "${TEST_LOG}"' EXIT

set +e
"${GODOT_EXECUTABLE}" --headless --path "${PROJECT_DIR}" \
  --quit-after 300 \
  --script res://tests/presentation3d/run_camera_rig_3d_test.gd 2>&1 \
  | tee "${TEST_LOG}"
...
if grep -Eq 'SCRIPT ERROR|Parse Error|Compile Error|Failed to load script|Invalid call|Stack overflow|^ERROR:|resources still in use|ObjectDB instances were leaked' "${TEST_LOG}"; then
  echo "Camera-rig 3D test detected a Godot script/runtime/resource error." >&2
  exit 1
fi
exit "${GODOT_STATUS}"
