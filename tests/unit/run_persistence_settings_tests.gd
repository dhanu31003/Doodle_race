extends SceneTree
## Execute with tests/unit/run_persistence_settings_tests.sh so parse/script
## errors are treated as failures even if the engine exits with status zero.

const SUITES: Array[Script] = [
	preload("res://tests/unit/test_persistence_settings.gd"),
	preload("res://tests/unit/test_profile_exchange.gd"),
]
	...
	for suite_script in SUITES:
		...
		if int(suite_result.get("assertions", 0)) == 0:
			...
			for failure in suite_result["failures"]:
				print("  - %s" % failure)
	print("Persistence/settings tests: %d assertions, %d failed suites" % [assertion_count, failed_suites])
