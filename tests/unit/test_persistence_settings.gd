...
func _test_accessibility_runtime_contracts(test: RefCounted) -> void:
	...
	test.assert_equal(NetworkRaceScreenType.steering_surface_for_scheme(GameSettingsType.CONTROL_BUTTONS), &"wheel", "legacy button saves migrate to the analog steering wheel")
...
func _test_settings_schema(test: RefCounted) -> void:
	...
	test.assert_equal(settings.touch_control_scheme, GameSettingsType.CONTROL_WHEEL, "unknown touch scheme must fall back to mobile wheel steering")
